...
def getMockEnvironmentConfig(
    environment,
    version="qwerty123456",
    readonly=False,
    pullrequest="foo.bar/pr/123456789qwerty",
):
...
class TestBitbucketCloud(asynctest.TestCase):
    ...
    @mock.patch("devops_sccs.realtime.hookserver.uvicorn")
    async def test05_get_bitbucket_repositories_bitbucket_session(
        self, mock_uvicorn, mock_bitbucket
    ):
        print(5)
        # Arrange
        self.core = await SccsCore.create(self.config)
        ...
        bitbucket_instance.user.permissions.repositories.get.return_value = (
            AsyncIterator([rep1, rep2])
        )
    ...
    # @mock.patch('devops_sccs.plugins.bitbucketcloud.Bitbucket')
    @mock.patch("devops_sccs.realtime.hookserver.uvicorn")
    async def test06_fetch_continuous_deployment_config_should_return_branch_available(
        self, mock_uvicorn
    ):
        ...
        # Test
        result = await self.bitbucketPlugin._fetch_continuous_deployment_config(
            **{"repository": self.args["test_repo"]["name"]}
        )
    ...
    async def test07_handle_push_from_bitbucket(self):
                ...
                "changes": [
                    {
                        ...
                        "new": {
                            "name": "deploy/dev",
                            "target": {"message": "deploy version f00ddeadbeeff00d"},
                        },
                    }
                ]
            },
        ...
        async with AsyncClient() as client:
            response = await client.post(
                self.hookPath + path, headers=headers, json=push_payload
            )

            # Assert
            # import pdb; pdb.set_trace()
            environementConfigResults = self.bitbucketPlugin.cache[
    ...
    # @mock.patch('devops_sccs.plugins.bitbucketcloud.Bitbucket')
    @mock.patch("devops_sccs.realtime.hookserver.uvicorn")
    async def test08_Given_get_continuous_deployment_config_When_another_user_get_same_repo_Then_data_should_be_get_from_cache(
        self, mock_uvicorn
    ):
        self.core = await SccsCore.create(self.config)
        await self.bitbucketPlugin.init(self.core, self.args)
        ...
        original = await self.bitbucketPlugin.get_continuous_deployment_config(
            session, testRepo
        )

        edited = await self.bitbucketPlugin.cache["continuousDeploymentConfig"][
            testRepo
        ]
        ...
        result = await self.bitbucketPlugin.get_continuous_deployment_config(
            session2, testRepo
        )
    ...
    @pytest.mark.anyio
    async def test09_Given_get_continuous_deployment_config_When_handle_push_deploydev_Then_data_should_be_get_updated_in_cache(
        self,
    ):
        self.core = await SccsCore.create(self.config)
        await self.bitbucketPlugin.init(self.core, self.args)
                ...
                "changes": [
                    {
                        ...
                        "new": {
                            "name": "deploy/dev",
                            "target": {"message": f"deploy version {version}"},
                        },
                    }
                ]
            },
    ...
    @mock.patch("devops_sccs.realtime.hookserver.uvicorn")
    @pytest.mark.anyio
    async def test10_Given_cache_ref_class_containing_it_When_sent_to_other_thread_Then_should_sync_multi_thread(
        self, mock_uvicorn
    ):
        print(10)
        cache_key = "a"
        ...
        def assert_value(expected_value, environement):
            ...
            self.assertEqual(
                result,
                expected_value,
                f'"{result}" is not equal "{expected_value}" in {environement}',
            )
        ...
        async with AsyncClient(
            app=app_test, base_url="http://localhost:5002"
        ) as client:
